...
def create_patient(patient: Patient):
    ...
    if patient.id in data:          # check if the patient ID already exists in the database
        raise HTTPException(status_code = 400, detail = "Patient with this ID already exists.")

    data[patient.id]= patient.model_dump(exclude =['id'])    # add new patient to the database
...
def update_patient(patient_id: str, patient_update: PatientUpdate): 
    ...
    updated_patient_info = patient_update.model_dump(exclude_unset=True)  # Get only the fields that are provided in the update request

    for key, value in updated_patient_info.items():
        existing_patient_info[key] = value

    existing_patient_info['id'] = patient_id  # Ensure the ID remains unchanged
    patient_pydantic_obj = Patient(**existing_patient_info)  # Create a new Patient object to recalculate BMI and verdict
    existing_patient_info = patient_pydantic_obj.model_dump(exclude ='id')  # Update the existing patient info with recalculated values
